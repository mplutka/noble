...
Gatt.prototype.onAclStreamData = function (cid, data) {
  ...
  if (this._currentCommand && data.toString('hex') === this._currentCommand.buffer.toString('hex')) {
    ...
    /*if (valueHandle === 61) {
      // Skip write responses
       console.error("MP gatt onAcl still got 61 reponses");
       return;
    } */
...
Gatt.prototype.notify = function (serviceUuid, characteristicUuid, notify, valueHandle) {
  ...
  this._queueCommand(this.readByTypeRequest(characteristic.startHandle, characteristic.endHandle, GATT_CLIENT_CHARAC_CFG_UUID), data => {
    const opcode = data[0];
    //if (valueHandle === 61) 
      //console.log("MP gatt notify opcode", valueHandle, notify, data)
    ...
    if (opcode === ATT_OP_READ_BY_TYPE_RESP) {
      ...
      //if (valueHandle === 61)
        //console.log("MP gatt notify response", valueHandle, handle, value, useNotify, useIndicate)
      ...
      //if (valueHandle === 61) 
      //console.log("MP gatt notify writeReqe", valueHandle, handle, valueBuffer);

      this._queueCommand(this.writeRequest(handle, valueBuffer, false), data => {
        const opcode = data[0];
        //if (valueHandle === 61) 
        //console.log("MP gatt notify writeReq Resp", valueHandle, handle, data);
