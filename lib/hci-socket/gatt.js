      ...
      for (const characteristicUuid in this._characteristics[serviceUuid]) {
        this._characteristics[serviceUuid][characteristicUuid].forEach((c, index) => {
          if (c.valueHandle === valueHandle) {
            this.emit('notification', this._address, serviceUuid, characteristicUuid, valueData, index);
          }
        })
...
Gatt.prototype.read = function (serviceUuid, characteristicUuid, characteristicIndex) {
  const characteristic = this._characteristics[serviceUuid][characteristicUuid][characteristicIndex];
  ...
  const callback = (data) => {
    ...
    if (opcode === ATT_OP_READ_RESP || opcode === ATT_OP_READ_BLOB_RESP) {
      ...
      if (data.length === this._mtu) {
        ...
        this.emit('read', this._address, serviceUuid, characteristicUuid, readData, characteristicIndex);
      }
    } else {
      this.emit('read', this._address, serviceUuid, characteristicUuid, readData, characteristicIndex);
    }
