...
NobleBindings.prototype.read = function (peripheralUuid, serviceUuid, characteristicUuid, characteristicIndex) {
  const handle = this._handles[peripheralUuid];
  const gatt = this._gatts[handle];

  if (gatt) {
    gatt.read(serviceUuid, characteristicUuid, characteristicIndex);
  } else {
    console.warn(`noble warning: unknown peripheral ${peripheralUuid}`);
  }
};

NobleBindings.prototype.onRead = function (address, serviceUuid, characteristicUuid, data, characteristicIndex) {
  const uuid = address.split(':').join('').toLowerCase();

  this.emit('read', uuid, serviceUuid, characteristicUuid, data, false, characteristicIndex);
};
...
NobleBindings.prototype.onNotification = function (address, serviceUuid, characteristicUuid, data, characteristicIndex) {
  const uuid = address.split(':').join('').toLowerCase();
  this.emit('read', uuid, serviceUuid, characteristicUuid, data, true, characteristicIndex);
};
