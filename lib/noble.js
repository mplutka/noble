...
Noble.prototype.read = function (peripheralUuid, serviceUuid, characteristicUuid, characteristicIndex) {
  this._bindings.read(peripheralUuid, serviceUuid, characteristicUuid, characteristicIndex);
};

Noble.prototype.onRead = function (peripheralUuid, serviceUuid, characteristicUuid, data, isNotification, characteristicIndex) {
  const characteristic = this._characteristics[peripheralUuid][serviceUuid][characteristicUuid][characteristicIndex];

  if (characteristic) {
    characteristic.emit('data', data, isNotification, characteristicIndex);

    characteristic.emit('read', data, isNotification, characteristicIndex); // for backwards compatbility
  } else {
    this.emit('warning', `unknown peripheral ${peripheralUuid}, ${serviceUuid}, ${characteristicUuid}, ${characteristicIndex} read!`);
  }
};
